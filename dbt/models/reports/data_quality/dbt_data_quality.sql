WITH null_counts AS (
  SELECT
    ...
    COUNT(DISTINCT CASE WHEN is_fake_discount IS TRUE THEN name END) AS fake_discount_product_count,
...
SELECT
  ...
  SAFE_DIVIDE(nc.null_prev_price, nc.total_rows) AS percent_null_prev_price,
  ...
  SAFE_DIVIDE(nc.null_prev_original_price, nc.total_rows) AS percent_null_prev_original_price,
  SAFE_DIVIDE(nc.null_discount_price, nc.total_rows) AS percent_null_discount_price,
  SAFE_DIVIDE(nc.null_price_var_abs, nc.total_rows) AS percent_null_price_var_abs,
  SAFE_DIVIDE(nc.null_price_var_percent, nc.total_rows) AS percent_null_price_var_percent,
  SAFE_DIVIDE(nc.null_original_price_var_abs, nc.total_rows) AS percent_null_original_price_var_abs,
  SAFE_DIVIDE(nc.null_original_price_var_percent, nc.total_rows) AS percent_null_original_price_var_percent,
  SAFE_DIVIDE(nc.null_is_fake_discount, nc.total_rows) AS percent_null_is_fake_discount,
  nc.null_dedup_id,
  nc.null_name,
  nc.null_size,
  nc.null_category,
